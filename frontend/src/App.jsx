...
const GENERATE_COMM_URL = 
  "https://ai-resume-reviewer-e412.onrender.com/api/generate-communication";
...
export default function App() {
  ...
  // =========================================================
  // BATCH 1: COVER LETTER & COLD EMAIL STATE
  // =========================================================
  ...
  const handleGenerateComm = async () => {
    ...
    try {
      const formData = new FormData();
      formData.append("structured_data", JSON.stringify(structured));
      formData.append("job_description", jobDescription);

      const response = await axios.post(GENERATE_COMM_URL, formData, {
      ...
      if (response.data?.success) {
        setCommData({
          ...
          coldEmail: response.data.cold_email,
        });
      } else {
        setCommError("Failed to generate communication templates.");
      }
    } catch (err) {
      console.error("Communication Generation Error:", err);
      setCommError("Unable to reach the AI server for generating templates. Please try again.");
    } finally {
                ...
                >
                  ...
                  <div className="mt-6">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div>
                        <label className="text-sm font-bold text-white">
                          🎯 Target Job Description or URL
                        </label>

                        <p className="text-xs text-slate-500 mt-1">
                          Paste a real job posting or a direct link (LinkedIn, Indeed, etc.) for personalized matching.
                        </p>
                      </div>
                    ...
                    <textarea
                      ...
                      placeholder={`Paste the job description OR a Job Link (https://...) here...
...
REST APIs...`}
                    ...
                    <p className="mt-2 text-xs text-slate-600">
                      💡 Tip: A real job posting or a URL link gives better match insights.
                    </p>
          ...
          {structured && !loading && (
            ...
            <section className="mt-12">
              ...
              {/* =========================================================
                  FEATURE 1 (BATCH 1): OUTREACH & APPLICATION (COVER LETTER)
              ========================================================= */}
              <div className="mb-8 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 md:p-8 backdrop-blur-xl">
                <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 border-b border-slate-800 gap-4 mb-6">
                  <div>
                    <div className="inline-flex items-center gap-2 px-3 py-1 mb-2 rounded-full border border-purple-500/30 bg-purple-500/10 text-purple-300 text-xs font-semibold">
                      ✉️ Smart Application Outreach
                    </div>
                    <h3 className="text-xl md:text-2xl font-black text-white">
                      1-Click Cover Letter & Cold Email
                    </h3>
                    <p className="text-xs md:text-sm text-slate-400 mt-1">
                      Generate highly personalized application templates based on your resume strengths and the target role.
                    </p>
                  </div>
                  <div>
                    <button
                      ...
                      className="px-5 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 font-bold text-white shadow-lg shadow-purple-500/20 hover:brightness-110 transition disabled:opacity-60 flex items-center gap-2"
                    >
                      {generatingComm ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-500">
                    ...
                    <div className="bg-slate-950/70 border border-slate-800 rounded-2xl p-5">
                      <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-800">
                        ...
                        <button 
                          onClick={() => { navigator.clipboard.writeText(commData.coverLetter); alert("Cover Letter Copied!"); }}
                          className="text-xs text-indigo-400 hover:text-indigo-300 font-semibold"
                        >
                          📋 Copy
                        </button>
                      </div>
                      <div className="text-xs text-slate-300 leading-relaxed whitespace-pre-wrap h-64 overflow-y-auto custom-scrollbar pr-2">
                        {commData.coverLetter}
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-950/70 border border-slate-800 rounded-2xl p-5">
                      <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-800">
                        <h4 className="text-sm font-bold text-white flex items-center gap-2">
                          <span>📧</span> Hiring Manager Cold Email
                        </h4>
                        <button 
                          onClick={() => { navigator.clipboard.writeText(commData.coldEmail); alert("Cold Email Copied!"); }}
                          className="text-xs text-indigo-400 hover:text-indigo-300 font-semibold"
                        >
                          📋 Copy
                        </button>
